...
class Graph:
    ...
    def tarjanSCC(self):
        ...
        def doDFS(node, time):
            time += 1
            nodes_status[node]['discovery'] = nodes_status[node]['low'] = time
            stack.append(node)
            nodes_status[node]['instack'] = True
            adj_vertices = self.getVertex(node).getConnections()
            for adj_vertex in adj_vertices:
                #Do DFS if forward-edge
                if nodes_status[adj_vertex.id]['discovery'] == -1:
                    doDFS(adj_vertex.id, time)
                    nodes_status[node]['low'] = min(nodes_status[node]['low'], nodes_status[adj_vertex.id]['low'])
                #Check if back-edge
                elif nodes_status[adj_vertex.id]['instack']:
                    nodes_status[adj_vertex.id]['low'] = min(nodes_status[node]['low'], nodes_status[adj_vertex.id]['discovery'])
            #If no neighbour, do backtracking
            if nodes_status[node]['discovery'] == nodes_status[node]['low']:
                print('SCC is: \n')
